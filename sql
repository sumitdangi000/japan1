...
WITH DynamicDates AS (
    SELECT 
        MAX(CASE 
            WHEN DATEPART(QUARTER, Asofdate) = DATEPART(QUARTER, DATEADD(QUARTER, -1, GETDATE())) 
                 AND YEAR(Asofdate) = YEAR(DATEADD(QUARTER, -1, GETDATE())) 
            THEN Asofdate 
        END) AS PrevQtrEndDate,
        MAX(CASE 
            WHEN MONTH(Asofdate) = MONTH(DATEADD(MONTH, -1, GETDATE())) 
                 AND YEAR(Asofdate) = YEAR(DATEADD(MONTH, -1, GETDATE())) 
            THEN Asofdate 
        END) AS LastMonthEndDate,
...
PrevQtrData AS (
    ...
    FROM 
        ...
        DynamicDates
    WHERE 
        Asofdate = (SELECT PrevQtrEndDate FROM DynamicDates)
        AND ProjectType IN ('P', 'PRODUCTION')
    GROUP BY MasterCustomerCode
),
LastMonthData AS (
    ...
    FROM 
        ...
        DynamicDates
    WHERE 
        Asofdate = (SELECT LastMonthEndDate FROM DynamicDates)
        AND ProjectType IN ('P', 'PRODUCTION')
    GROUP BY MasterCustomerCode
),
CombinedData AS (
